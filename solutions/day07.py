...
def first_order(hand: str):
    ...
    e.g. AAAAA results in {"A": 5}
    Therefore the set of values in the count result is {5}, and the order given is 7
    """
    count = Counter(hand)
    set_count = set(count.values())
    if set_count == {5}:
        ord = 7  # Five of a kind AAAAA
    elif set_count == {1, 4}:
        ord = 6  # Four of a kind AA8AA
    elif set_count == {2, 3}:
        ord = 5  # Full house 23332
    elif set_count == {1, 3}:
        ord = 4  # Three of a kind TTT98
    elif set_count == {1, 2}:
        if len(count) == 3:
            ord = 3  # Two pair 23432
        else:
            ord = 2  # One pair A23A4
    elif set_count == {1}:
        ord = 1  # High card 23456
    return ord
...
def add_ordering(hands: List[str]):
    ...
    return [
        (hand, bet, [first_order(hand)] + second_order(hand, translator))
...
def part1(i: List[str]):
    ...
    then uses enumerate to find the rank, and calculates the bet * rank
    """
    hands_with_order = add_ordering(i)
    total = 0
    for rank, (_, bet, _) in enumerate(
        sorted(hands_with_order, key=lambda x: x[2]), start=1
    ):
        total += rank * int(bet)
    return total
